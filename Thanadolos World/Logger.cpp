...
void Logger::NetworkRcv(Packet &packet, WorldClient &client, std::string name, int color)
{
	Locker *lock = Logger::getLocker();
	lock->lock();
	HANDLE hConsole = GetStdHandle(STD_OUTPUT_HANDLE);
...
void Logger::NetworkSnd(IMessage *message, Packet &packet, WorldClient &client, int color)
{
	Locker *lock = Logger::getLocker();
	lock->lock();
	HANDLE hConsole = GetStdHandle(STD_OUTPUT_HANDLE);
...
void Logger::NetworkIPC(IMessage *message, Packet &packet, int color)
{
	Locker *lock = Logger::getLocker();
	lock->lock();
	HANDLE hConsole = GetStdHandle(STD_OUTPUT_HANDLE);
	SetConsoleTextAttribute(hConsole, color);
	std::cout << "[IPC] packet sent : " << message->getName() \
		<< ", length: " << packet.length() << " bytes to auth server." << std::endl;
	lock->unlock();
}